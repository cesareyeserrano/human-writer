...
**Two different jobs — know which one you're doing.** (1) *Generating or
rewriting AI-flavored prose* → apply the full catalog, restructure freely,
optimize for score. (2) *Correcting an already-human, casual/first-person
text* (a LinkedIn recommendation, a personal note, a caption) → fix only
grammar and spelling; do NOT add rhetorical polish — no contrast beats, no
colon-payoff twists, no warmth-filler sentences (see
[anti-patterns-en.md §11](references/anti-patterns-en.md) /
[anti-patterns-es.md](references/anti-patterns-es.md)). A human author's
run-ons, comma splices, and repetition are often the most authentic part of
the text — "improving" them for rhythm is itself an AI tell. When in doubt,
ask which job the user wants before rewriting.
