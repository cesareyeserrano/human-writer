...
### B2b. When editing (not generating), don't upgrade for rhetorical effect

A distinct failure mode from B2, seen in field use: given an already-human,
casual/first-person text to *correct*, the agent "fixes" clauses that
weren't broken — adding a contrast beat, a colon-payoff twist, or a
warmth-filler sentence that states no new information. The result reads
more polished and *more* AI-flavored, even with zero banned phrases and a
low scorer number, because the tell is structural (every clause engineered
to land) rather than lexical. See anti-patterns-en.md §11 / anti-patterns-es.md
§11. A comma splice, a repeated word, or a dangling fragment in a casual
first-person register (a recommendation, a text, a caption) is often the
most human part of the text — leave it. Fix only grammar and spelling in
that mode; ask the user if unsure which mode they want.
